...
function BlueprintVisual({ project }: { project: Project }) {
  return (
    <div className="relative w-full overflow-hidden" style={{ aspectRatio: "16/10", background: "#0D0D0D", border: "1px solid #222" }}>
      ...
      {/* Tech pills */}
      <div className="absolute bottom-0 left-0 right-0 p-5 flex flex-wrap gap-2">
      ...
      {/* Corner brackets */}
      <div className="absolute top-4 right-4 w-3 h-3 border-t border-r" style={{ borderColor: "rgba(201,184,154,0.5)" }} />
      <div className="absolute bottom-4 left-4 w-3 h-3 border-b border-l" style={{ borderColor: "rgba(201,184,154,0.5)" }} />
    </div>
  );
}
...
function ProjectVisual({ project }: { project: Project }) {
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ border: "1px solid #222" }}>
      {/* Browser chrome */}
      <div className="flex items-center gap-1.5 px-3" style={{ height: 28, background: "#111", borderBottom: "1px solid #222" }}>
        <span className="w-2 h-2 rounded-full" style={{ background: "#333" }} />
        <span className="w-2 h-2 rounded-full" style={{ background: "#333" }} />
        <span className="w-2 h-2 rounded-full" style={{ background: "#333" }} />
        <div className="flex-1 rounded-sm mx-2 flex items-center px-2" style={{ height: 14, background: "#1A1A1A" }}>
          <span className="font-mono truncate" style={{ fontSize: "8px", color: "#666" }}>
      ...
      <div style={{ aspectRatio: "16/9", background: "#0A0A0A", overflow: "hidden", position: "relative" }}>
        ...
        <img src={screenshotSrc} alt={`${project.name} preview`} loading="lazy"
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <span className="font-mono text-[10px] tracking-[0.3em] uppercase" style={{ color: "#C9B89A" }}>{num}</span>
        <span className="font-mono text-[10px] tracking-[0.2em] uppercase" style={{ color: "#777" }}>{project.track}</span>
        <span className="font-mono text-[10px] tracking-[0.2em] uppercase" style={{ color: "#555" }}>{project.year}</span>
      </div>

      {/* Name */}
      <h3 className="font-condensed font-black leading-none mb-4"
        style={{ fontSize: "clamp(2.2rem, 4.5vw, 4rem)", color: "#F5F5F2", letterSpacing: "-0.02em" }}>
        {project.name}
      </h3>

      {/* Tagline */}
      <p className="mb-6 leading-relaxed" style={{ fontSize: "0.95rem", color: "#999", maxWidth: "38ch" }}>
        {project.tagline}
      </p>

      {/* Description */}
      <p className="mb-8 leading-relaxed" style={{ fontSize: "0.82rem", color: "#777", maxWidth: "42ch" }}>
        {project.description}
      </p>
      ...
      <div className="flex items-center gap-5">
        <div className="flex items-center gap-2">
          <span className="inline-block w-1.5 h-1.5 rounded-full" style={{ background: STATUS_COLOR[project.status] }} />
...
function ExhibitRow({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={{ borderColor: "#222", paddingTop: "6rem", paddingBottom: "6rem" }}>

      {/* Watermark number */}
      <span className="absolute font-condensed font-black select-none pointer-events-none leading-none"
        style={{
          fontSize: "28vw",
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center ${!isEven ? "md:[direction:rtl]" : ""}`}>
        <div className={!isEven ? "md:[direction:ltr]" : ""}><InfoBlock project={project} num={num} /></div>
        <div className={!isEven ? "md:[direction:ltr]" : ""}><ProjectVisual project={project} /></div>
      </div>
    </motion.div>
  );
}
...
function RestList({ projects }: { projects: Project[] }) {
  return (
    <div className="mt-24">
      <div className="flex items-center gap-6 mb-8" style={{ borderBottom: "1px solid #222", paddingBottom: "1.5rem" }}>
      ...
      <div>
        {projects.map((project, i) => (
          <motion.div key={project.id}
            ...
            <span className="font-mono text-[9px] shrink-0 w-6" style={{ color: "#555" }}>
              {String(featured.length + i + 1).padStart(2, "0")}
            </span>

            <span className="font-condensed font-bold transition-colors"
              style={{ fontSize: "1.25rem", color: "#999", letterSpacing: "-0.01em", minWidth: "200px" }}
              onMouseEnter={e => (e.currentTarget.style.color = "#F5F5F2")}
              onMouseLeave={e => (e.currentTarget.style.color = "#999")}>
              {project.name}
            </span>

            <span className="font-mono text-[9px] tracking-widest uppercase hidden md:block"
              style={{ color: "#555", minWidth: "100px" }}>
              {project.track}
            </span>

            <span className="flex-1 text-[0.8rem] hidden lg:block leading-snug" style={{ color: "#777" }}>
              {project.tagline}
            </span>

            <span className="font-mono text-[9px]" style={{ color: "#666" }}>{project.year}</span>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-4 md:gap-16">
            <h2 className="font-condensed font-black leading-none"
            ...
            <p className="font-mono leading-relaxed max-w-xs"
              style={{ fontSize: "9px", color: "#777", paddingBottom: "0.5rem", letterSpacing: "0.1em" }}>
        ...
        <div style={{ borderTop: "1px solid #222" }}>
          {featured.map((p, i) => <ExhibitRow key={p.id} project={p} index={i} />)}
        </div>
